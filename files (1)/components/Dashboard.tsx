...
import {
  ...
  X, Wand2, FileText, ShieldCheck, Coins, Edit3, Tag, Copy
} from 'lucide-react';
...
function hasCoins(profile: UserProfile | null): boolean {
  if (!profile) return false;
  if (profile.coinsBalance === -1) return true;
  return (profile.coinsBalance ?? 0) >= 1;
}

async function deductCoins(uid: string, prompt: string, response: string) {
  const cost = Math.max(1, Math.ceil((prompt.length + response.length) / 750));
  await updateDoc(doc(db, 'users', uid), {
    coinsBalance: increment(-cost),
    coinsUsed: increment(cost),
  });
}

function coinDisplay(balance: number): string {
  if (balance === -1) return '∞';
...
export default function Dashboard({ user, profile, appName, logoUrl }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [bSnap, dcSnap] = await Promise.all([
          getDocs(query(collection(db, 'books'), where('userId', '==', user.uid))),
          getDocs(query(collection(db, 'discount_codes'), where('isActive', '==', true))),
        ]);
        const list = bSnap.docs.map(d => ({ id: d.id, ...d.data() } as Book));
        list.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
        setBooks(list);
        setDiscountCodes(dcSnap.docs.map(d => ({ id: d.id, ...d.data() })));
  ...
  const generateStory = async () => {
    ...
    if (!hasCoins(profile)) { toast.error('Insufficient coins. Contact admin to top up.'); return; }
    setGenerating(true); setGenResult('');
    try {
      const sys = `You are DreamForgeAI, a master children's book storyteller. Write a vivid story in the "${genStyle}" visual style. Structure it with clear pages (Page 1:, Page 2:). Each page: 2-3 paragraphs. Warm, age-appropriate, imaginative.`;
      const text = await generateWithGemini(genPrompt, sys);
      setGenResult(text);
      await deductCoins(user.uid, genPrompt, text);
  ...
  if (editingBook) {
    return <BookEditor book={editingBook} user={user} profile={profile} onClose={() => setEditingBook(null)} />;
  }
    ...
    <div className="min-h-screen bg-night text-white">
      <header className="border-b border-white/6 px-6 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {/* Coin balance */}
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gold/10 border border-gold/15">
              <Coins size={14} className="text-gold" />
              <span className="text-gold text-xs font-bold">{coinDisplay(coins)}</span>
              <span className="text-gold/50 text-xs">coins</span>
            </div>
            ...
            <span className="text-white/40 text-sm hidden sm:block">{displayName}</span>
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        <div className="mb-10">
          ...
          <p className="text-white/40 text-sm">{profile ? `Level ${profile.level} · ${profile.xp} XP · ${profile.subscriptionTier} plan · ${coinDisplay(coins)} coins` : 'Ready to create'}</p>
        </div>

        {/* Active Discount Codes Banner */}
        {discountCodes.length > 0 && (
          <div className="mb-8 glass-card rounded-xl p-5">
            <div className="flex items-center gap-2 mb-3"><Tag size={15} className="text-gold" /><span className="font-semibold text-sm">Active Promotions</span></div>
            <div className="flex flex-wrap gap-2">
              {discountCodes.map(dc => (
                <div key={dc.id} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gold/8 border border-gold/15">
                  <code className="text-gold font-mono text-xs tracking-wider">{dc.code}</code>
                  <span className="text-white/40 text-xs">
                    {dc.discountType === 'percent' ? `${dc.discountValue}% off` : `$${dc.discountValue} off`}
                    {dc.tier !== 'any' ? ` ${dc.tier}` : ''}
                  </span>
                  <button onClick={() => { navigator.clipboard.writeText(dc.code); toast.success('Code copied!'); }} className="text-white/20 hover:text-gold ml-1"><Copy size={11} /></button>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Low coins warning */}
        {profile && profile.coinsBalance !== -1 && (profile.coinsBalance ?? 0) < 10 && (
          <div className="mb-8 rounded-xl p-4 border border-amber-500/20 bg-amber-500/5">
            <p className="text-amber-400 text-sm">Low coins ({coinDisplay(coins)} remaining). Contact an admin to top up your balance.</p>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {books.map(b => (
              <div key={b.id} className="glass-card p-5 group flex flex-col">
                ...
                <h3 className="font-semibold text-sm mb-1 flex-1">{b.title}</h3>
                <p className="text-white/30 text-xs capitalize mb-4">{b.type} · {b.status}</p>
                <button
                  onClick={() => setEditingBook(b)}
                  className="w-full flex items-center justify-center gap-2 py-2 rounded-xl border border-gold/20 text-gold/80 text-xs font-medium hover:bg-gold/10 transition-all"
                >
                  <Edit3 size={13} /> Edit Book
                </button>
              </div>
            ))}
          </div>
      ...
      {showNew && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative rounded-2xl p-8 w-full max-w-sm" style={{ background: '#13121a', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <input type="text" placeholder="Book title" value={newTitle} onChange={e => setNewTitle(e.target.value)} className="input-dark mb-4" autoFocus onKeyDown={e => e.key === 'Enter' && createBook()} />
      ...
      {showGen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative rounded-2xl p-8 w-full max-w-lg max-h-[80vh] overflow-y-auto" style={{ background: '#13121a', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            {/* Coin cost reminder */}
            <div className="flex items-center gap-2 mb-5 px-3 py-2 rounded-lg bg-white/3 border border-white/6">
              <Coins size={13} className="text-gold" />
              <span className="text-white/40 text-xs">Costs ~2–5 coins. Balance: <span className="text-gold font-bold">{coinDisplay(coins)}</span></span>
            </div>
            {!genResult ? (
              <>
                ...
                <div className="mb-6">
                  ...
                  <select value={genStyle} onChange={e => setGenStyle(e.target.value)} className="input-dark" style={{ colorScheme: 'dark' }}>
                    ...
                    <option value="cartoon">Whimsical Cartoon</option>
                    <option value="anime">Anime / Ghibli</option>
                    <option value="sketch">Pencil Sketch</option>
                    <option value="comic">Comic Book</option>
                    <option value="midnight-tales">Midnight Tales</option>
                    <option value="neon-noir">Neon Noir</option>
                  </select>
                </div>
                <button onClick={generateStory} disabled={generating || !genPrompt.trim() || !hasCoins(profile)} className="btn-gold w-full flex items-center justify-center gap-2">
                  {generating ? <><Loader2 size={16} className="animate-spin" /> Generating...</> : <><Wand2 size={16} /> Generate Story</>}
                </button>
                {!hasCoins(profile) && <p className="text-amber-400 text-xs text-center mt-2">Insufficient coins.</p>}
              </>
            ) : (
              <>
